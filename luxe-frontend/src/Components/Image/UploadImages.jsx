...
import { MdCloudUpload, MdOutlinePhotoLibrary, MdInfoOutline, MdCategory } from 'react-icons/md';
...
const UploadImages = () => {
  ...
  const [fileName, setFileName] = useState("No visual detected");
  ...
  const inputImageChangeHandler = (e) => {
    ...
    if (name === "images") {
      if (files && files[0]) {
        setAllImage({ ...allImage, images: files[0] });
        setFileName(files[0].name);
      }
    } else {
      setAllImage({ ...allImage, [name]: value });
    }
  };
  ...
  const handleImageSubmit = async (e) => {
    e.preventDefault();
    if (!allImage.images) {
      toast.error("Please initiate a visual signal first");
      return;
    }
    try {
      ...
      await axios.post("http://localhost:7000/users/uploadimage", formData, { withCredentials: true });
      navigate("/imagehome");
      toast.success("Visual broadcast successful!");
    } catch (err) {
      console.error("Upload error:", err);
      toast.error("Failed to broadcast visual");
    }
  };
        ...
        <div className="w-full max-w-[800px] bg-[#0a0c10]/40 backdrop-blur-3xl border border-white/5 rounded-[3.5rem] p-12 md:p-20 shadow-2xl relative group overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-16 text-center">
            <div className="h-16 w-16 bg-purple-600/10 rounded-3xl flex items-center justify-center mb-6 border border-purple-500/10 shadow-[0_0_30px_rgba(147,51,234,0.1)]">
              <MdOutlinePhotoLibrary className="text-3xl text-purple-500" />
            </div>
            <h1 className="text-5xl font-black mb-4 tracking-tighter uppercase">
              Curate <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-purple-600">Gallery</span>
            </h1>
            <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.4em] opacity-60">Prepare your high-fidelity visual capture</p>
          </div>

          <form onSubmit={handleImageSubmit} className="space-y-10">
            <div className="space-y-8">
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1 flex items-center gap-2">
                  <MdInfoOutline className="text-purple-500" /> Visual Identity
                </label>
                <input 
                  ...
                  type="text" 
                  placeholder="The title of your capture..." 
                  name="title" 
                  required 
                  value={allImage.title} 
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1 flex items-center gap-2">
                  <MdInfoOutline className="text-purple-500" /> Artistic Vision
                </label>
                <textarea 
                  ...
                  placeholder="Describe the soul of this capture..." 
                  name="description" 
                  required 
                  value={allImage.description} 
                  onChange={inputImageChangeHandler}
                />
              </div>
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1 flex items-center gap-2">
                  <MdCategory className="text-purple-500" /> Aesthetic Classification
                </label>
                <input 
                  className="w-full bg-white/[0.03] border border-white/5 rounded-2xl py-5 px-8 outline-none focus:border-purple-500/30 transition-all text-[14px] font-bold placeholder:text-gray-800 shadow-inner" 
                  type="text" 
                  placeholder="e.g. Minimal, Abstract, Cinematic" 
                  name="category" 
                  required 
                  value={allImage.category} 
                  onChange={inputImageChangeHandler}
                />
              </div>
            </div>
            ...
            <div className="bg-[#050608]/50 rounded-3xl p-8 border border-dashed border-white/10 flex flex-col items-center gap-6 group/upload transition-all hover:border-purple-500/30">
              <label className="cursor-pointer w-full flex flex-col items-center gap-4">
                ...
                <div className="text-center">
                  <span className="text-[11px] font-black uppercase tracking-[0.2em] text-white/80">Select Visual Protocol</span>
                  <p className="text-[9px] text-gray-600 font-bold mt-1 uppercase tracking-widest">Supports high-fidelity image formats</p>
                </div>
                <input 
                  type="file" 
                  name="images" 
