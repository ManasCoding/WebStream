...
import { MdEdit, MdInfoOutline, MdPersonOutline, MdAccountCircle } from 'react-icons/md';
...
const EditPage = () => {
  ...
  const handleUpdate = async function (e) {
    e.preventDefault();
    try {
      await axios.post("http://localhost:7000/users/updateProfile", {
        channel: user.channel,
        name: user.name,
        about: user.about,
      }, { withCredentials: true });

      toast.success("Profile evolution complete!");
        ...
        <div className="w-full max-w-[700px] bg-[#0a0c10]/40 backdrop-blur-3xl border border-white/5 rounded-[3.5rem] p-12 md:p-16 shadow-2xl relative group overflow-hidden">
          ...
          <div className="flex flex-col items-center mb-16 text-center">
            <div className="h-16 w-16 bg-purple-600/10 rounded-3xl flex items-center justify-center mb-6 border border-purple-500/10 shadow-[0_0_30px_rgba(147,51,234,0.1)]">
              <MdEdit className="text-3xl text-purple-500" />
            </div>
            <h1 className="text-5xl font-black mb-4 tracking-tighter uppercase">
              Refine <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-purple-600">Identity</span>
            </h1>
            <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.4em] opacity-60">Evolve your presence in the media universe</p>
          </div>
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1 flex items-center gap-2">
                  <MdAccountCircle className="text-purple-500" /> Channel Designation
                </label>
                <input 
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1 flex items-center gap-2">
                  <MdPersonOutline className="text-purple-500" /> Authorized Operator
                </label>
                <input 
                  className="w-full bg-white/[0.03] border border-white/5 rounded-2xl py-5 px-8 outline-none focus:border-purple-500/30 transition-all text-[14px] font-bold placeholder:text-gray-800 shadow-inner" 
                  type="text" 
                  placeholder="Your legal designation..." 
                  name="name" 
                  required 
                  value={user.name} 
                  onChange={handleChange}
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-500 ml-1 flex items-center gap-2">
                  <MdInfoOutline className="text-purple-500" /> Mission Statement
                </label>
                <textarea 
                  className="w-full bg-white/[0.03] border border-white/5 rounded-3xl py-6 px-8 outline-none focus:border-purple-500/30 transition-all text-[14px] font-bold placeholder:text-gray-800 min-h-[140px] resize-none shadow-inner leading-relaxed" 
                  placeholder="Define your purpose and vision..." 
                  name="about" 
                  required 
                  value={user.about} 
                  onChange={handleChange}
