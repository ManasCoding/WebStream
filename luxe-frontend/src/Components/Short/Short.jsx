...
const Short = ({ top = "top-12" }) => {
    ...
    const getUserShort = async () => {
        try {
            const response = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:7000'}/users/getusershorts`, { withCredentials: true });
                    ...
                    {usershort.map((short, index) => (
                        ...
                        >
                            <div className="relative aspect-[9/16] rounded-xl overflow-hidden bg-zinc-800">
                                <video
                                    src={`${import.meta.env.VITE_API_URL || 'http://localhost:7000'}${short.shorts}`}
